...
my @list_out		= make_list_out(\%runIO);		# List of WBM variables to output
        ### control model control based on requested output
my $advanceStop         = get_timestep_stop(\@list_out);
my $calcBalance         = ($advanceStop > 0) ? 1 : 0;
   $noState             = 1     if ($advanceStop == 0);
...
#   snowBandBalance($snowBand,$elevBand,$indxBand, $snowPack);		# Check balance of snow bands
  ...
  my $resStoragePrev    = $zeroes       ->copy;
  if ($advanceStop > 0) {
     $dischargePrev	= $discharge	->copy;
     $resStoragePrev	= $resStorage[0]->copy;
   }
...
#   map{ $riceSoilWater	+= $irrFactor * $irr_appRte{$_} if m/^rice/i } sort keys %{$land{irrCrop}}; # Not yet used in output
  ...
  if ($advanceStop == 0) {
      # At the end of soil moisture calculation #GOTO OUTPUT ;
      goto Output;
  }
	###############################################################
	#####   Baseflow and Runoff
 ...
 Output:
  ...
  if ($yrEndCond) {
    ...
    map $_=0,	$runoffT_y,   $RunoffT_y, $dischT_y, $precipT_y, $etNonCropsT_y, $etIrrCropsT_y, $etRfdCropsT_y,
      ...
      printf " %s (%s) %s\n", $path, $date,$sp_str ; }# Calc Balances
	###############################################################
	###   Clear previous day
...
#######################################################################
sub get_timestep_stop
{
    # Identifies the point within each daily time-step when further
    #  calculations are no-longer needed for the requested output
    # This is built with the expectation that multiple time-steps could
    #  be defined.  Presently - will end after soil moisture is updated
    #  Or will continue through estimation of discharge.
    my $advanceStop = 0;
    my $outputs = shift();
    if (grep( /^runoff/, @$outputs) | grep( /^discharge/,@$outputs)){
	$advanceStop = 2;
    }
    if ($advanceStop == 0) {
	print "WBM being run for soil moisture evaluation only.  No water balance checks can be calculated.\n";
	    }
    return $advanceStop;
