...
for(i in 1:5000){
  ...
  mean_value <- mean(SM_Output[indices,j])
  
  # Count the number of days above the mean
  days_above_mean <- sum(SM_Output[indices,j] > mean_value)
  output_5000[i,j]<- days_above_mean
  }}
...
write.table(output_5000, file="/storage/group/pches/default/users/aqa6478/aqa6478/WBM/Codes/SA/D_Above_AV_SM_GS_Stations/data/model_output.txt", row.names=FALSE, col.names=FALSE)
