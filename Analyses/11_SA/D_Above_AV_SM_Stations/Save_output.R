...
for(i in 1:5000){
  ...
  mean_value <- mean(SM_Output[,j])
  
  # Count the number of days above the mean
  days_above_mean <- sum(SM_Output[,j] > mean_value)
...
write.table(output_5000, file="/storage/group/pches/default/users/aqa6478/aqa6478/WBM/Codes/SA/D_Above_AV_SM_Stations/data/model_output.txt", row.names=FALSE, col.names=FALSE)
